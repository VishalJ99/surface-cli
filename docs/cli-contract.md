...
- `surface auth login <account> [--remember-me] [--remote-host <host>] [--remote-project-dir <path>]`
...
- Outlook v1 may require `account identity set` because the web-profile auth probe does not yet
  reliably extract the authenticated mailbox email
- summary-producing commands use stored account identity automatically; they do not accept
  per-command ME flags
- email aliases are for deterministic envelope matching, and name aliases help summary models
  interpret body references such as "Vishal, can you..."

Example account identity result:
...
```json
{
  ...
  "identity": {
    "account_id": "acc_01K...",
    "primary_email": "v.jain24@imperial.ac.uk",
    "display_name": "Vishal Jain",
    "email_aliases": [],
    "name_aliases": ["Vishal", "Jain, Vishal"],
    "primary_email_source": "user_confirmed",
    "display_name_source": "user_confirmed",
    "verified_at": "2026-04-21T12:51:00Z",
    "updated_at": "2026-04-21T12:51:00Z"
  }
}
```
...
- `surface auth login <account> --remember-me` records remembered-auth metadata in the project
...
- With `--remote-host`, `--remember-me` writes the remembered-auth metadata on the remote host after
  remote auth succeeds. By default Surface writes to `.env` under the same path as the current local
  working directory; use `--remote-project-dir <path>` when the remote checkout lives elsewhere.
...
- Remote `--remember-me` requires the chosen remote project directory to already exist; Surface does
  not create arbitrary remote checkout directories.
...
- Without an account, it checks all configured accounts. With `--remembered-only`, it checks only
  accounts named in `SURFACE_REMEMBERED_AUTH_ACCOUNTS`.
...
- `summary`
  ...
  The public shape is stable whether the summary was freshly generated or reused from a
  matching stored fingerprint. `summary.needs_action` is a triage signal judged from the selected
  account owner's perspective when account identity is configured; it is not an execution command.
...
- repeated `search`, `fetch-unread`, and `thread get --refresh` calls may reuse a stored
  summary when the normalized thread content, account-owner identity semantics, prompt version, and
  clipping policy are unchanged for the same summarizer backend/model
